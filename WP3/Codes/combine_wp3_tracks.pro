...
; Name          : combine_wp3_tracks

; Purpose       : Create the WP3 catalogue output necessary for the online catalogue, by combining the tracks of CME lists (that were generated by calling combine_wp3_lists.pro) and outputting the necessary final format.

; Explanation   : The environment variable HELCATS is used to point the codes to the necessary directory. 

; Use           : IDL> combine_wp3_tracks
...
; Prev. Hist.   : Original code final2.pro written by David Barnes.
...
PRO combine_wp3_tracks, test=test
	
	print, 'Running combine_wp3_tracks.pro'
        dir = getenv('HELCATS') ; '/soft/ukssdc/share/Solar/HELCATS' 
	dir_catalog = getenv('HI_CATALOGUE') ; dir+'/HI_catalogue'
	dir_wp2 = getenv('WP2_CATALOGUE') ; dir+'/WP2_catalogue'
	dir_wp3 = getenv('WP3_CATALOGUE') ; dir+'/WP3_catalogue'
	dir_tracks = getenv('HI_TRACKS') ; dir+'/tracks'

	LOAD_STEREO_SPICE

	print, 'dir: ', dir
	print, 'dir_catalog: ', dir_catalog
	print, 'dir_wp2: ', dir_wp2
	print, 'dir_wp3: ', dir_wp3
	...
	t = (keyword_set(test)) ? '_test' : ''
  
	FOR c = 0,1 DO BEGIN
		craft = crafts[c]
		print, 'Spacecraft: ', craft
		ids = STRARR(2000)
		j = 0
		infile2 = dir_wp2+'/STEREO-'+craft+'_CME_LIST_WP2'+t+'.txt'
		OPENR,102,infile2
		...
		ENDWHILE
		CLOSE,102
		outfile = dir_wp3+'/STEREO-'+craft+'_CME_LIST_WP3'+t+'.txt'
		OPENW,101,outfile
